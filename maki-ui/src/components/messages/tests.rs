...
use crate::components::test_ui_config;
...
fn search_text_includes_role_prefix() {
    ...
    assert_eq!(texts[0], format!("{}hello", user_style().prefix));
    assert_eq!(texts[2], format!("{}{md}", assistant_style().prefix));
    assert_eq!(texts[4], format!("{}hmm", thinking_style().prefix));
}
...
fn panel_with_long_tool(line_count: usize) -> MessagesPanel {
    let body = (0..line_count)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let mut ui_config = test_ui_config();
...
fn handle_click_on_watched_tool_sends_click_with_fallback(is_error: bool) {
    ...
    let mut panel = MessagesPanel::new(test_ui_config(), eh);
